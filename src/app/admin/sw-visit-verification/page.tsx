...
export default function SWVisitVerificationPage() {
  ...
  const submitVisitVerification = async () => {
    ...
    if (!selectedRCFE) {
      toast({
        title: "Error",
        description: "Please select an RCFE facility",
        variant: "destructive"
      });
      return;
    }
    ...
    if (visitedCount === 0) {
      toast({
        title: "Error",
        description: "Please check off at least one member as visited",
        variant: "destructive"
      });
      return;
    }
    ...
    */
      const verification: VisitVerification = {
        ...
        rcfeId: selectedRCFE,
        rcfeName: rcfeFacilities.find(f => f.id === selectedRCFE)?.name || '',
        visitDate,
        visitTime,
      ...
      console.log('📝 Submitting visit verification:', verification);
      
      toast({
        title: "Visit Verification Submitted",
        description: `Visit to ${verification.rcfeName} has been recorded with ${visitedCount} members visited`,
      });
      ...
      console.error('Error submitting visit verification:', error);
      toast({
        title: "Submission Failed",
        description: "Failed to submit visit verification. Please try again.",
        variant: "destructive"
      });
    }
  };

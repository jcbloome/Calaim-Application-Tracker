...
export async function GET(request: NextRequest) {
  try {
    ...
    const memberName = searchParams.get('memberName') || 'Jacqueline';

    // Search for applications with the member name
    ...
    // Filter for the specific member
    const matchingApps = allApps.filter(app => 
      app.memberFirstName?.toLowerCase().includes(memberName.toLowerCase())
    );

    return NextResponse.json({
      success: true,
      totalApplications: allApps.length,
      matchingApplications: matchingApps.length,
      applications: matchingApps.map(app => ({
        id: app.id,
        path: app.path,

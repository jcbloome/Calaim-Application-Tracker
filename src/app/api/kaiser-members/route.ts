...
const toDraftKaiserMember = (docId: string, app: Record<string, unknown>) => {
  ...
  return {
    ...
    Next_Auth_Start_T2038: String(app?.Next_Auth_Start_T2038 || '').trim(),
    Next_Auth_End_T2038: String(app?.Next_Auth_End_T2038 || '').trim(),
  ...
  try {
    ...
    if (!preferCaspio) {
      ...
      const transformedMembers = cached.map((member: any) => ({
        ...
        ISP_Location_Type: pickFirstPopulated(member, ['ISP_Location_Type', 'Current_Location_Type', 'Location_Type']),
        ISP_Contact_Location: pickFirstPopulated(member, ['ISP_Contact_Location', 'ISP_Current_Location', 'RCFE_Name', 'Facility_Name']),
        ISP_Contact_First: pickFirstPopulated(member, ['ISP_Contact_First', 'Contact_First']),
        ISP_Contact_Last: pickFirstPopulated(member, ['ISP_Contact_Last', 'Contact_Last']),
        ISP_Contact_Address: pickFirstPopulated(member, ['ISP_Contact_Address', 'ISP_Current_Address', 'Member_Address', 'Address']),
        ISP_Contact_City: pickFirstPopulated(member, ['ISP_Contact_City', 'ISP_Current_City', 'Member_City', 'MemberCity', 'City']),
        ISP_Contact_State: pickFirstPopulated(member, ['ISP_Contact_State', 'ISP_Current_State', 'Member_State', 'State']),
        ISP_Contact_Zip: pickFirstPopulated(member, ['ISP_Contact_Zip', 'ISP_Current_Zip', 'Member_Zip', 'Zip']),
        ISP_Contact_Name: pickFirstPopulated(member, ['ISP_Contact_Name', 'Contact_Name', 'RCFE_Admin_Name']),
        ...
        Authorization_Start_Date_H2022: member.Authorization_Start_Date_H2022 || '',
        Authorization_End_Date_H2022: member.Authorization_End_Date_H2022 || '',
        Authorization_End_Date_T2038: member.Authorization_End_Date_T2038 || member.Authorization_End_T2038 || '',
        Next_Auth_Start_T2038: member.Next_Auth_Start_T2038 || '',
        Next_Auth_End_T2038: member.Next_Auth_End_T2038 || '',
        Need_More_Contact_Info_ILS: member.Need_More_Contact_Info_ILS || '',
    ...
    // using explicit pageSize/pageNumber pagination so we do not truncate at 500.
    console.log('📊 Fetching Kaiser members with robust pagination...');

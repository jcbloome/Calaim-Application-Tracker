...
function applyPreviewFormatting(field: string, value: string): string {
  ...
  if (field === 'p2_current_state' || field === 'p2_home_state') {
...
function pickMappedValue(
  ...
  const forced = HOME_ADDRESS_OVERRIDES[targetField];
  const primary = forced || clean(mappingRows?.[targetField]?.primary, 200);
  if (!primary) return '';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const mappedFields = Object.values(mappingRows || {})
      ...
      .filter(Boolean);
    const forcedHome = Object.values(HOME_ADDRESS_OVERRIDES);

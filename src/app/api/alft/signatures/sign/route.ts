...
async function generateSignaturePagePdf(args: {
  ...
  mrn?: string | null;
  reviewedAtMs?: number;
  rnRecommendedTier?: string | null;
  adminApprovedTier?: string | null;
  ...
  // Tier recommendation / admin approval at bottom of signature page
  const rnTier = String(args.rnRecommendedTier || '').trim();
  const adminTier = String(args.adminApprovedTier || '').trim();
  if (rnTier || adminTier) {
    y -= 8;
    page.drawRectangle({ x: marginX, y: y - 2, width: 516, height: 18, color: rgb(0.94, 0.92, 0.98) });
    page.drawText('Tier recommendation', { x: marginX + 6, y: y + 1, size: 12, font: fontBold, color: dark });
    y -= 28;
    if (rnTier) {
      page.drawText(`RN recommended tier: Tier ${rnTier}`, { x: marginX, y, size: 11, font: fontBold, color: dark });
      y -= 18;
    }
    if (adminTier) {
      const by = String(args.adminApprovedByName || '').trim();
      page.drawText(
        `Admin approved tier: Tier ${adminTier}${by ? ` (approved by ${by})` : ''}`,
        { x: marginX, y, size: 11, font: fontBold, color: dark }
      );
      y -= 18;
    }
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (bothSigned && (!signaturePageReady || !packetReady)) {
      ...
      let intakeForPacket: any = null;
      if (intakeId) {
        try {
          const intakeSnapForPacket = await adminDb.collection('standalone_upload_submissions').doc(intakeId).get();
          intakeForPacket = intakeSnapForPacket.exists ? (intakeSnapForPacket.data() as any) : null;
        } catch {
          intakeForPacket = null;
        }
      }
      ...
      const signaturePdfBytes = await generateSignaturePagePdf({
        ...
        rnRecommendedTier:
          clean(intakeForPacket?.alftRnTierRecommendation?.tier, 10) ||
          clean(intakeForPacket?.alftForm?.exactPacketAnswers?.p14_rn_recommended_tier, 10) ||
          clean(rnRecommendedTier, 10) ||
          null,
        adminApprovedTier:
          clean(intakeForPacket?.alftManagerReview?.rnRecommendedTier, 10) ||
          clean(intakeForPacket?.alftForm?.exactPacketAnswers?.p14_admin_approved_tier, 10) ||
          null,
        adminApprovedByName: clean(intakeForPacket?.alftManagerReview?.reviewedByName, 160) || null,
        rn: {
      ...
      try {
        ...
        if (originalUrl) {
          ...
          if (intakeForPacket) {
            try {
              const att = intakeForPacket?.alftForm?.medListAttachment || null;

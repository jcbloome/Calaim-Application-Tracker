...
import {
  buildIspDailyActionReminderEmailContent,
  sendIspDailyActionReminderEmail,
...
type Body = {
  ...
  /** When true, resolve recipient + draft email without sending. */
  preview?: boolean;
...
async function resolveReminderContext(params: {
  adminDb: any;
  memberId: string;
  targetRole: 'auto' | 'msw' | 'rn';
}) {
  const { adminDb, memberId, targetRole } = params;
  const assignmentRef = adminDb.collection('alft_assignments').doc(memberId);
  const assignmentSnap = await assignmentRef.get();
  if (!assignmentSnap.exists) {
    return { error: 'ISP assignment not found for this member', status: 404 as const };
  }
  ...
  if (!needed.recipientEmail || !needed.recipientEmail.includes('@')) {
    return {
      error: `Missing email for ${needed.role === 'msw' ? 'social worker' : needed.role}`,
      status: 409 as const,
    };
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const resolved = await resolveReminderContext({ adminDb, memberId, targetRole });
    ...
    if (previewOnly) {
      return NextResponse.json({
        ...
        defaultNextAction: needed.nextAction,
        actionUrl: draft.actionUrl,
        memberName,
        mrn: mrn || '',
        subject: draft.subject,
        defaultSubject: `ISP action needed: ${memberName} — ${needed.stageLabel}`,
    ...
    await sendIspDailyActionReminderEmail(emailPayload);

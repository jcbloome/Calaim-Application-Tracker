...
import { PrintableFullPackageVietnamese } from '@/components/forms/PrintableFullPackageVietnamese';
...
function VietnameseFullPackageContent() {
  ...
  const viewerHref = useMemo(() => {
    ...
    params.set('view', 'pdf');
    return `/forms/printable-package/vietnamese?${params.toString()}`;
  ...
  return (
    <PdfPreviewLayout
      ...
      captureContent={
        <PrintableFullPackageVietnamese
      ...
      pdfError={pdfError}
      previewTitle="Vietnamese full package PDF preview"
...
export default function VietnameseFullPackagePrintPage() {
  return (
    <div className="min-h-screen bg-gray-50 print:bg-white">
      <main className="container mx-auto py-8 px-4 print:p-0">
        <Suspense fallback={<div className="flex justify-center items-center h-64">Loading...</div>}>
          <VietnameseFullPackageContent />
        </Suspense>
        <div className="mt-4 text-center text-sm print:hidden">
          <Button asChild variant="outline">
            <Link href="/forms/printable-package/vietnamese?view=pdf">Open PDF Preview</Link>
          </Button>
        </div>
      </main>
    </div>
